...
def public_school_website(request):
    """Renders the official public-facing marketing homepage for Educlique"""
...
def student_registry_workstation(request):
    # 1. Define the true primary school grade structure for Educlique School
    ...
    for student in all_students:
        ...
        if balance > 0:
            defaulters_queue.append({
                'student': student,
                'balance': balance,
                'message_preview': f"Dear Parent, please note Educlique records show an outstanding balance of KES {balance:,.2f} for Adm {student.admission_number}."
            })
...
def _load_students_from_csv():
    csv_path = os.path.join(settings.BASE_DIR, '..', 'Educlique School - STUDENTS.csv')
